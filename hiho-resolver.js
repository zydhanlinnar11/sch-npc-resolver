...
Resolver.prototype.status = function (problem) {
  if (problem.old_verdict == 'NA' && problem.new_verdict == 'NA')
    return 'untouched'
  else if (problem.old_verdict == 'AC') return 'ac'
  else if (problem.new_verdict == 'NA') return 'failed'
  else return 'frozen'
}
...
Resolver.prototype.calcOperations = function () {
  ...
  for (var solution_id in this.solutions) {
    ...
    if (sol.submitted_seconds <= this.frozen_seconds) {
      this.rank[sol.user_id].problem[sol.problem_index].old_verdict =
        sol.verdict
      if (sol.verdict == 'AC') {
        this.rank[sol.user_id].problem[sol.problem_index].old_submissions++
        this.rank[sol.user_id].problem[sol.problem_index].ac_penalty =
          sol.submitted_seconds
        this.rank[sol.user_id].problem[sol.problem_index].old_penalty =
          this.rank[sol.user_id].problem[sol.problem_index].ac_penalty +
          20 *
            60 *
            (this.rank[sol.user_id].problem[sol.problem_index].old_submissions -
        ...
        this.rank[sol.user_id].penalty +=
          this.rank[sol.user_id].problem[sol.problem_index].old_penalty
      } else {
        this.rank[sol.user_id].problem[sol.problem_index].old_submissions++
      }
    } else {
      //after standings get frozen
      if (
        this.rank[sol.user_id].problem[sol.problem_index].new_verdict == 'AC'
      ) {
        this.rank[sol.user_id].problem[sol.problem_index].frozen_submissions++
        continue
      }
      this.rank[sol.user_id].problem[sol.problem_index].new_verdict =
        sol.verdict
      if (sol.verdict == 'AC') {
        this.rank[sol.user_id].problem[sol.problem_index].frozen_submissions++
        this.rank[sol.user_id].problem[sol.problem_index].new_submissions =
          this.rank[sol.user_id].problem[sol.problem_index].old_submissions +
          this.rank[sol.user_id].problem[sol.problem_index].frozen_submissions
        this.rank[sol.user_id].problem[sol.problem_index].ac_penalty =
          sol.submitted_seconds
        this.rank[sol.user_id].problem[sol.problem_index].new_penalty =
          this.rank[sol.user_id].problem[sol.problem_index].ac_penalty +
          20 *
            60 *
            (this.rank[sol.user_id].problem[sol.problem_index].new_submissions -
    ...
    while (flag) {
      ...
      for (var j = 1; j <= this.problem_count; j++) {
        if (this.status(this.rank2[i].problem[j]) == 'frozen') {
          ...
          while (
            k >= 0 &&
            (this.rank2[k].score < tmp.score ||
              (this.rank2[k].score == tmp.score &&
                this.rank2[k].penalty > tmp.penalty))
          ) {
